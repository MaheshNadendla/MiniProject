...
function MyStatus() {
  ...
  if (loading || myStatuses.length === 0) return (
        ...
        <div className='OthersStatusPhotoBox'>
          ...
          >
            <div className='OthersStatusPhoto'>
              <img src={ authUser?.profilePic || 'defaultImg.png'} alt='profile' />
            </div>
          </div>
        </div>

        <div className='OthersStatusMessagesInfo'>
          <div className='OthersMyStatusName'>{ `MyStatus (${authUser.name})` }</div>
          <div className='OthersStatusClick'>{`click here for more info`}</div>
        </div>
